...
# -*- coding: utf-8 -*-
"""
Turbo Network Engine v2.2 - Complete System
MikroTik & Generic Support | All Bugs Fixed
"""
import requests
import re
import urllib3
...
def check_real_internet():
    """Check if real internet is accessible (with hardened timeout)"""
    try:
        # Set a socket-level timeout as a safety net
...
# ===============================
# MIKROTIK DETECTION & AUTH
# ===============================
def detect_mikrotik(response_text, response_url):
    """Heuristic to identify MikroTik captive portal"""
...
def extract_login_form(response_text, base_url):
    """Extract login form action and hidden fields from MikroTik page"""
    action = ""
    data = {}
...
def mikrotik_auth(session, portal_url, sid):
    """Perform POST authentication on MikroTik hotspot"""
    try:
        ...
        # Insert session ID appropriately
...
# ===============================
# HIGH SPEED PING (WiFiDog mode)
# ===============================
def high_speed_ping(auth_link, sid, session):
    """Keep-alive pings for WiFiDog-style portals"""
    ping_count = 0
    success_count = 0
...
def start_turbo_engine():
    """Main engine with MikroTik and WiFiDog support"""
    stop_event.clear()  # FIX: Reset the event so the loop can run again
    os.system('clear' if os.name == 'posix' else 'cls')
    display_banner()
    print(f"{bcyan}╔══════════════════════════════════════════════════════════════════╗")
    print(f"║                    TURBO NETWORK ENGINE v2.2                        ║")
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            r1 = session.get(portal_url, verify=False, timeout=10)
            # Capture session ID from all known patterns
            sid = None
            ...
            # Decide between MikroTik and generic WiFiDog
            ...
            if is_mikrotik:
                ...
                while not stop_event.is_set():
                    ...
                    time.sleep(2)
                # When internet drops or Ctrl+C, loop continues to re-detect
